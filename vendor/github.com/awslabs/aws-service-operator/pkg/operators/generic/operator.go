...
func NewOperator(config config.Config, queueManager *queuemanager.QueueManager) *Operator {
	...
	topicARN, _ := queuectrl.Register("generic")
...
func (c *Operator) StartWatch(ctx context.Context, namespace string) {
	...
	oper := operator.New("generics", namespace, resourceHandlers, c.config.AWSClientset.RESTClient())
	oper.Watch(&awsV1alpha1.GenericTopic{}, ctx.Done())
}
...
func QueueUpdater(config config.Config, msg *queuemanager.MessageBody) error {
	...
	if msg.Updatable {
		...
		resources, err := clientSet.GenericTopics("").List(metav1.ListOptions{})
		if err != nil {
			logger.WithError(err).Error("error getting generics")
			return err
		}
...
func (c *Operator) onAdd(obj interface{}) {
	s := obj.(*awsV1alpha1.GenericTopic).DeepCopy()
	if s.Status.ResourceStatus == "" || s.Status.ResourceStatus == "DELETE_COMPLETE" {
		...
		if err != nil {
			c.config.Logger.WithError(err).Errorf("error creating generic '%s'", s.Name)
			return
		}
		c.config.Logger.Infof("added generic '%s' with stackID '%s'", s.Name, string(*output.StackId))
...
func (c *Operator) onUpdate(oldObj, newObj interface{}) {
	oo := oldObj.(*awsV1alpha1.GenericTopic).DeepCopy()
	no := newObj.(*awsV1alpha1.GenericTopic).DeepCopy()
	...
	if helpers.IsStackComplete(oo.Status.ResourceStatus, false) && !reflect.DeepEqual(oo.Spec, no.Spec) {
		...
		if err != nil {
			c.config.Logger.WithError(err).Errorf("error updating generic '%s' with new params %+v and old %+v", no.Name, no, oo)
			return
		}
		c.config.Logger.Infof("updated generic '%s' with params '%s'", no.Name, string(*output.StackId))
		c.config.Logger.Infof("view at https://console.aws.amazon.com/cloudformation/home?#/stack/detail?stackId=%s", string(*output.StackId))
...
func (c *Operator) onDelete(obj interface{}) {
	...
	err := cft.DeleteStack()
	if err != nil {
		c.config.Logger.WithError(err).Errorf("error deleting generic '%s'", s.Name)
		return
	}

	c.config.Logger.Infof("deleted generic '%s'", s.Name)
}
func incrementRollbackCount(config config.Config, name string, namespace string) error {
	logger := config.Logger
	clientSet, _ := awsclient.NewForConfig(config.RESTConfig)
	resource, err := clientSet.GenericTopics(namespace).Get(name, metav1.GetOptions{})
	if err != nil {
		logger.WithError(err).Error("error getting generics")
		return err
	}
	...
	_, err = clientSet.GenericTopics(namespace).Update(resourceCopy)
	if err != nil {
...
func updateStatus(config config.Config, name string, namespace string, stackID string, status string, reason string) (*awsV1alpha1.GenericTopic, error) {
	logger := config.Logger
	clientSet, _ := awsclient.NewForConfig(config.RESTConfig)
	resource, err := clientSet.GenericTopics(namespace).Get(name, metav1.GetOptions{})
	if err != nil {
	...
	if helpers.IsStackComplete(status, false) {
		...
		resourceCopy.Output.GenericARN = outputs["GenericARN"]
	}
...
func deleteStack(config config.Config, name string, namespace string, stackID string) (*awsV1alpha1.GenericTopic, error) {
	logger := config.Logger
	clientSet, _ := awsclient.NewForConfig(config.RESTConfig)
	resource, err := clientSet.GenericTopics(namespace).Get(name, metav1.GetOptions{})
	if err != nil {
...
func syncAdditionalResources(config config.Config, s *awsV1alpha1.GenericTopic) (err error) {
	clientSet, _ := awsclient.NewForConfig(config.RESTConfig)
	resource, err := clientSet.GenericTopics(s.Namespace).Get(s.Name, metav1.GetOptions{})
	if err != nil {
		return err
	}
	resource = resource.DeepCopy()

	_, err = clientSet.GenericTopics(s.Namespace).Update(resource)
	if err != nil {
		return err
	}
	return nil
