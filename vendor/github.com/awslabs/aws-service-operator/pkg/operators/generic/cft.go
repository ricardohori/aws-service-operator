...
func New(config config.Config, generic *awsV1alpha1.GenericTopic, topicARN string) *Cloudformation {
...
// Cloudformation defines the generic cfts
type Cloudformation struct {
	config       config.Config
	GenericTopic *awsV1alpha1.GenericTopic
	topicARN     string
}

// StackName returns the name of the stack based on the aws-operator-config
func (s *Cloudformation) StackName() string {
	return helpers.StackName(s.config.ClusterName, "generic", s.GenericTopic.Name, s.GenericTopic.Namespace)
}
...
func (s *Cloudformation) CreateStack() (output *cloudformation.CreateStackOutput, err error) {
	...
	cftemplate := helpers.GetCloudFormationTemplate(s.config, "generic", s.GenericTopic.Spec.CloudFormationTemplateName, s.GenericTopic.Spec.CloudFormationTemplateNamespace)
	...
	resourceName := helpers.CreateParam("ResourceName", s.GenericTopic.Name)
	resourceVersion := helpers.CreateParam("ResourceVersion", s.GenericTopic.ResourceVersion)
	namespace := helpers.CreateParam("Namespace", s.GenericTopic.Namespace)
	...
	resourceNameTag := helpers.CreateTag("ResourceName", s.GenericTopic.Name)
	resourceVersionTag := helpers.CreateTag("ResourceVersion", s.GenericTopic.ResourceVersion)
	namespaceTag := helpers.CreateTag("Namespace", s.GenericTopic.Namespace)
...
func (s *Cloudformation) UpdateStack(updated *awsV1alpha1.GenericTopic) (output *cloudformation.UpdateStackOutput, err error) {
	sess := s.config.AWSSession
	svc := cloudformation.New(sess)

	cftemplate := helpers.GetCloudFormationTemplate(s.config, "generic", updated.Spec.CloudFormationTemplateName, updated.Spec.CloudFormationTemplateNamespace)
